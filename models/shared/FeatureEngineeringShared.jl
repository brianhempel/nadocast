...
# And the larger mean blocks are depended on the smaller mean blocks.
# So not all feature combos are valid.
...
function feature_i_to_name(inventory :: Vector{Inventories.InventoryLine}, layer_blocks_to_make :: Vector{Int64}, feature_i :: Int64) :: String
...
function is_wind_key(key, vector_wind_layers)
...
function make_data(
  ...
  non_wind_feature_is = filter(feature_i -> !is_wind_key(feature_keys[feature_i], vector_wind_layers), 1:length(feature_keys))
  ...
  out = Array{Float32}(undef, (grid_point_count, length(layer_blocks_to_make)*raw_feature_count + 1))
  ...
  if twenty_five_mi_mean_block in layer_blocks_to_make
    twenty_five_mi_mean_features_range = block_i*raw_feature_count+1:(block_i+1)*raw_feature_count
    block_i += 1
  end

  if fifty_mi_mean_block in layer_blocks_to_make
    fifty_mi_mean_features_range = block_i*raw_feature_count+1:(block_i+1)*raw_feature_count
    block_i += 1
  end

  if hundred_mi_mean_block in layer_blocks_to_make
    hundred_mi_mean_features_range = block_i*raw_feature_count+1:(block_i+1)*raw_feature_count
    block_i += 1
  end

  if twenty_five_mi_forward_gradient_block in layer_blocks_to_make
    twenty_five_mi_forward_gradient_range = block_i*raw_feature_count+1:(block_i+1)*raw_feature_count
    block_i += 1
  end

  if twenty_five_mi_leftward_gradient_block in layer_blocks_to_make
    twenty_five_mi_leftward_gradient_range = block_i*raw_feature_count+1:(block_i+1)*raw_feature_count
    block_i += 1
  end

  if twenty_five_mi_linestraddling_gradient_block in layer_blocks_to_make
    twenty_five_mi_linestraddling_gradient_range = block_i*raw_feature_count+1:(block_i+1)*raw_feature_count
    block_i += 1
  end

  if fifty_mi_forward_gradient_block in layer_blocks_to_make
    fifty_mi_forward_gradient_range = block_i*raw_feature_count+1:(block_i+1)*raw_feature_count
    block_i += 1
  end

  if fifty_mi_leftward_gradient_block in layer_blocks_to_make
    fifty_mi_leftward_gradient_range = block_i*raw_feature_count+1:(block_i+1)*raw_feature_count
    block_i += 1
  end

  if fifty_mi_linestraddling_gradient_block in layer_blocks_to_make
    fifty_mi_linestraddling_gradient_range = block_i*raw_feature_count+1:(block_i+1)*raw_feature_count
    block_i += 1
  end

  if hundred_mi_forward_gradient_block in layer_blocks_to_make
    hundred_mi_forward_gradient_range = block_i*raw_feature_count+1:(block_i+1)*raw_feature_count
    block_i += 1
  end

  if hundred_mi_leftward_gradient_block in layer_blocks_to_make
    hundred_mi_leftward_gradient_range = block_i*raw_feature_count+1:(block_i+1)*raw_feature_count
    block_i += 1
  end

  if hundred_mi_linestraddling_gradient_block in layer_blocks_to_make
    hundred_mi_linestraddling_gradient_range = block_i*raw_feature_count+1:(block_i+1)*raw_feature_count
    block_i += 1
  end

  forecast_hour_layer_i = block_i*raw_feature_count + 1
  ...
  if "UGRD:10 m above ground:hour fcst:wt ens mean" in vector_wind_layers
    # SREF
  ...
  mean_wind_lower_half_atmosphere_us .+= get_layer("UGRD:850 mb:hour fcst:wt ens mean")
  mean_wind_lower_half_atmosphere_vs .+= get_layer("VGRD:850 mb:hour fcst:wt ens mean")
  total_weight += 1.0f0

  mean_wind_lower_half_atmosphere_us .+= get_layer("UGRD:700 mb:hour fcst:wt ens mean")
  mean_wind_lower_half_atmosphere_vs .+= get_layer("VGRD:700 mb:hour fcst:wt ens mean")
  total_weight += 1.0f0

  if "UGRD:600 mb:hour fcst:wt ens mean" in vector_wind_layers
    # SREF
    mean_wind_lower_half_atmosphere_us .+= get_layer("UGRD:600 mb:hour fcst:wt ens mean")
    mean_wind_lower_half_atmosphere_vs .+= get_layer("VGRD:600 mb:hour fcst:wt ens mean")
    total_weight += 1.0f0
  end

  mean_wind_lower_half_atmosphere_us .+= get_layer("UGRD:500 mb:hour fcst:wt ens mean")
  mean_wind_lower_half_atmosphere_vs .+= get_layer("VGRD:500 mb:hour fcst:wt ens mean")
  total_weight += 1.0f0
