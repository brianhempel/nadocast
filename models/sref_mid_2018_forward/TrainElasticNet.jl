...
print("Loading")
X, y = get_data_and_labels(grid, conus_grid_bitmask, train_forecasts, X_transformer = (X -> Float64.(@view X[:, feature_is])))
...
# glmnet! bang version doesn't make a second copy of the data. It will be mutated for normalization.
path = GLMNet.glmnet!(X, Float64.(y), GLMNet.Binomial(), nlambda = 50, alpha = alpha, maxit = 100000)

X = nothing # freeeeeeeedom


println("Loading validation data")
validation_X, validation_y = get_data_and_labels(grid, conus_grid_bitmask, validation_forecasts, X_transformer = (X -> Float64.(@view X[:, feature_is])))
