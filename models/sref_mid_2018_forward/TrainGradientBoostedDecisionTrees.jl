...
function get_data_and_labels_binned(forecasts, bin_splits)
  ...
  SREFTrainShared.get_data_and_labels(grid, conus_grid_bitmask, forecasts, X_transformer = transformer)
end
...
(bin_sample_X, _) = SREFTrainShared.get_data_and_labels(grid, conus_grid_bitmask, Iterators.take(Random.shuffle(train_forecasts), bin_split_forecast_sample_count))
