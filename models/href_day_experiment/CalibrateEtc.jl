...
function do_it_all(forecasts, model_names, event_names, make_calibrated_hourly_models)
  ...
  inspect_predictive_power(validation_forecasts_with_blurs, (@view X[:,1:end-1]), Ys, weights, map(i -> model_names[i ÷ length(blurrers) + 1], 0:size(X,2)-1), map(i -> event_names[i ÷ length(blurrers) + 1], 0:size(X,2)-1))
