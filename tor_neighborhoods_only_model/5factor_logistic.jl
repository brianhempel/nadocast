...
function (a::Factors)(x)
  f1, f2, f3, f4, f5 = a.f1, a.f2, a.f3, a.f4, a.f5

  f1(x) .* f2(x) .* f3(x) .* f4(x) .* f5(x)
end

model = Factors(Dense(FEATURE_COUNT, 1, σ), Dense(FEATURE_COUNT, 1, σ), Dense(FEATURE_COUNT, 1, σ), Dense(FEATURE_COUNT, 1, σ), Dense(FEATURE_COUNT, 1, σ))
