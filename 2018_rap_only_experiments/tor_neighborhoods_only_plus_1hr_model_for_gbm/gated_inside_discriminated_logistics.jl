...
function (a::Logistics)(x)
  ...
  y =
    if dim1 < DIM1_SPLIT
      if dim2 < DIM2_SPLIT
        if dim3 < DIM3_SPLIT
          a.logistic000a(x) .* a.logistic000b(x)
        else
          a.logistic001a(x) .* a.logistic001b(x)
        end
      else
        if dim3 < DIM3_SPLIT
          a.logistic010a(x) .* a.logistic010b(x)
        else
          a.logistic011a(x) .* a.logistic011b(x)
        end
      end
    else
      if dim2 < DIM2_SPLIT
        if dim3 < DIM3_SPLIT
          a.logistic100a(x) .* a.logistic100b(x)
        else
          a.logistic101a(x) .* a.logistic101b(x)
        end
      else
        if dim3 < DIM3_SPLIT
          a.logistic110a(x) .* a.logistic110b(x)
        else
          a.logistic111a(x) .* a.logistic111b(x)
        end
      end
    end
