# Usage:
#
# Loads a trained model, applies it to a pair of adjacent grib2 forecasts (e.g. +5hr and +6hr), and plots the results.
#
# $ julia PredictAndPlot.jl tor_neighborhoods_only_model/2factor_logistic.jl tor_neighborhoods_only_model/2factor_logistic_2018-03-21T12-42-08.883_151545_examples_checkpoint_loss_0.0798001117753203.bson  rap_130_20170516_2200_005.grb2 rap_130_20170516_2200_006.grb2
#

include("PlotMap.jl")

if length(ARGS) < 3
  error("Must provide a model script and a saved model file and a two adjacent grib2 files (e.g. +5hr and +6hr)")
end


model_dir_and_type             = ARGS[1]
saved_model_path               = ARGS[2]
prior_hour_forecast_grib2_path = ARGS[3]
grib2_path                     = ARGS[4]
out_dir                        = length(ARGS) == 5 ? ARGS[5] : pwd()
...
data_stream, proc = open(`julia $MODEL_DIR/AdjacentGribsToFeatures.jl $prior_hour_forecast_grib2_path $grib2_path --all`)
