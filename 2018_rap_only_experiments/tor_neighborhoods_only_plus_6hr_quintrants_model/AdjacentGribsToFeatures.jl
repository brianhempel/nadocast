  ...
  for j = y_range
    ...
    for i = x_range
      ...
      if !IS_TRAINING || (lat_lon_to_key(lat, lon) in training_pts_set)
        ...
        data_between_25mi_and_56mi = data[:,flat_is_between_25mi_and_56mi]
        data_ahead  = data_between_25mi_and_56mi[:,indexin(flat_is_ahead,  flat_is_between_25mi_and_56mi)]
        data_behind = data_between_25mi_and_56mi[:,indexin(flat_is_behind, flat_is_between_25mi_and_56mi)]
        data_left   = data_between_25mi_and_56mi[:,indexin(flat_is_left,   flat_is_between_25mi_and_56mi)]
        data_right  = data_between_25mi_and_56mi[:,indexin(flat_is_right,  flat_is_between_25mi_and_56mi)]
        ...
        for abbrev_desc in regular_layer_order
          ...
          push!(out_row, mean(@view data_between_25mi_and_56mi[k,:]))
          ...
          if first_pt
            ...
            push!(headers, abbrev * ":" * desc * ":25mi-56mi ring mean")
        ...
        for winds_layer_i in 1:length(wind_layer_order)
          ...
          if abbrev[1] == 'U' || abbrev == "VUCSH" # Handle u and v layers together, so skip v layers.
            ...
            u_mean_between_25mi_and_56mi = mean(@view data_between_25mi_and_56mi[k,:])
            v_mean_between_25mi_and_56mi = mean(@view data_between_25mi_and_56mi[k+1,:])
            ...
            mean_r_between_25mi_and_56mi, mean_theta_between_25mi_and_56mi             = u_v_to_r_theta(u_mean_between_25mi_and_56mi, v_mean_between_25mi_and_56mi)
            ...
            push!(out_row, mean_r_between_25mi_and_56mi)
            relative_theta_between_25mi_and_56mi = relativize_angle(mean_theta_between_25mi_and_56mi, storm_theta)
            push!(out_row, relative_theta_between_25mi_and_56mi)
            # push!(out_row, abs(relative_theta_between_25mi_and_56mi))
            ...
            if first_pt
              if abbrev != "USTM"
                ...
                push!(headers, "R" * abbrev_root * ":$r_unit "                                              * desc * ":25mi-56mi ring mean")
                push!(headers, "T" * abbrev_root * ":angle radians from point storm motion "                * desc * ":25mi-56mi ring mean")
                # push!(headers, "T" * abbrev_root * ":absolute value angle radians from point storm motion " * desc * ":25mi-56mi ring mean")
                ...
                push!(headers, "R" * abbrev_root * ":$r_unit storm motion"                               * ":25mi-56mi ring mean")
                push!(headers, "T" * abbrev_root * ":angle radians from point storm motion storm motion" * ":25mi-56mi ring mean")
                # push!(headers, "T" * abbrev_root * ":absolute value angle radians from point storm motion storm motion" * desc * ":25mi-56mi ring mean")
